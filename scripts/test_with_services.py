...
def pre_test_checks():
    ...
    try:
        ...
        print("[ERROR] Docker is not running or not installed. Please start Docker Desktop or your Docker daemon.")
        sys.exit(1)
...
def wait_for_db():
    ...
    def main():
        import argparse
        parser = argparse.ArgumentParser(description="Run all tests with service setup/teardown.")
        ...
        parser.add_argument('--no-teardown', action='store_true', help='Do not tear down services after tests')
        ...
        backend_code = subprocess.call([
            './backend/.venv/bin/python', '-m', 'pytest', 'backend/tests', f'--junitxml={junit_path}'
        ] + pytest_args)
    ...
    code = subprocess.call([
        'npx', 'playwright', 'test', f'--reporter=junit,{junit_path}'
    ], cwd='e2e')
    if code != 0:
...
def main():
    ...
    parser.add_argument('--backend-files', nargs='+', help='Run only specified backend test files (space-separated)')
    parser.add_argument('--backend-keywords', help='Run only backend tests matching given pytest -k expression')
    parser.add_argument('--frontend-pattern', help='Run only frontend tests matching pattern (passed to vitest)')
    parser.add_argument('--e2e-pattern', help='Run only E2E tests matching pattern (passed to Playwright)')
    args, pytest_args = parser.parse_known_args()
    ...
    alembic_cmd = [
        './backend/.venv/bin/alembic', '-c', alembic_ini, 'upgrade', 'head'
    ]
