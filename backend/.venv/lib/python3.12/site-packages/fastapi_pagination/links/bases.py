...
TPage_contra = TypeVar(
    "TPage_contra", bound=AbstractPage, contravariant=True, default=Any
)
...
class BaseUseLinks(BaseLinksCustomizer[TPage_contra], ABC):
    ...
    def customize_page_ns(self, page_cls: PageCls, ns: ClsNamespace) -> None:
        if IS_PYDANTIC_V2:
            ...
            ns[self.field] = computed_field(return_type=Links)(
                lambda _self: self.resolve_links(_self)
            )
            return

        add_field = UseAdditionalFields(
            **{self.field: (Links, Field(default_factory=Links))}
        )
        add_field.customize_page_ns(page_cls, ns)

        @root_validator(skip_on_failure=True, allow_reuse=True)
        def __links_root_validator__(
            cls: Any, values: dict[str, Any]
        ) -> dict[str, Any]:  # noqa: N807
...
class BaseUseHeaderLinks(BaseLinksCustomizer[TPage_contra], ABC):
    ...
    def _customize_page_ns_pydantic_v1(
        self, page_cls: PageCls, ns: ClsNamespace, /
    ) -> None:
        @root_validator(skip_on_failure=True, allow_reuse=True)  # type: ignore[deprecated]
        def __add_links_to_header__(
            cls: Any, values: dict[str, Any]
        ) -> dict[str, Any]:  # noqa: N807
    ...
    def _customize_page_ns_pydantic_v2(
        self, page_cls: PageCls, ns: ClsNamespace, /
    ) -> None:
