...
_items_transformer_val: ContextVar[ItemsTransformer | None] = ContextVar(
    ...
    if params is None:
        try:
            ...
            raise UninitializedConfigurationError(
                "Use params, add_pagination or pagination_ctx"
            ) from None
...
def _create_params_dependency(
    ...
    async def _pagination_params(
        *args: Any, **kwargs: Any
    ) -> AsyncIterator[TAbstractParams_co]:
        if is_pydantic_v2_model:
            # should not happen cause all , but let's be safe
            if args:  # pragma: no cover
                raise ValueError(
                    "Positional arguments are not supported for Pydantic v2 models"
                )
    ...
    if IS_PYDANTIC_V2:
        ...
        if is_pydantic_v2_model:
            sign_params = [
                _get_param(name, field)
                for name, field in cast(BaseModel, params).model_fields.items()
            ]
