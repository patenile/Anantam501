...
def new_params_cls(
    cls: type[AbstractParams], new_ns: ClsNamespace
) -> type[AbstractParams]:
    new_cls = new_class(
        new_ns.get("__name__", cls.__name__),
        (cls,),
        ...
        def __class_getitem__(cls, item: Any) -> Any:
            ...
            for customizer in customizers:
                if not isinstance(customizer, (PageCustomizer, PageTransformer)):
                    raise TypeError(
                        f"Expected PageCustomizer or PageTransformer, got {customizer!r}"
                    )
            ...
            params_type = new_params_cls(
                cast(type[AbstractParams], new_ns["__params_type__"]),
                {"__page_type__": None},
            )
...
class _UseOptionalRequiredFields(PageCustomizer):
    ...
    def customize_page_ns(self, page_cls: PageCls, ns: ClsNamespace) -> None:
        ...
        fields_to_update = {
            name: field for name, field in fields.items() if name in self.fields
        }
        ...
        if self.required:
            fields_to_update = {
                k: make_field_required(v) for k, v in fields_to_update.items()
            }

            if is_pydantic_v2_model(page_cls):
                # to make field required in pydantic v2 we just need to update its type annotation
                customizer = UseFieldTypeAnnotations(
                    **{k: get_field_tp(v) for k, v in fields_to_update.items()}
                )
            else:
            ...
            if is_pydantic_v2_model(page_cls):
                fields_to_update = {
                    k: make_field_optional(v) or v for k, v in fields_to_update.items()
                }
                customizer = UseAdditionalFields(
                    **{
                        k: (create_annotated_tp(get_field_tp(v), v), None)
...
class UseOptionalParams(PageCustomizer):
    def customize_page_ns(self, page_cls: PageCls, ns: ClsNamespace) -> None:
        ...
        new_fields = {
            name: make_field_optional(field) for name, field in fields.items()
        }
    ...
    def customize_page_ns(self, page_cls: PageCls, ns: ClsNamespace) -> None:
        if not is_pydantic_v2_model(page_cls):  # pragma: no cover
            raise UnsupportedFeatureError(
                "UseResponseHeaders is only supported in Pydantic v2"
            )
        ...
        def model_post_init(_self: AbstractPage[Any], context: Any, /) -> None:
            ...
            for k, v in self.resolver(_self).items():
                match v:
                    ...
                    case _:
                        raise TypeError(
                            f"Header value must be str or list[str], got {v!r}"
                        )
...
def _convert_v2_page_cls_to_v1(page_cls: type[AbstractPage], /) -> BaseModelV1:
    ...
    *_, generic_base = get_page_bases(page_cls)
    ...
    new_cls = create_model_v1(  # type: ignore[call-overload]
        ...
        **{
            k: _convert_v2_field_to_v1(cast(FieldV2, v))
            for k, v in get_model_fields(page_cls).items()
        },
    )
