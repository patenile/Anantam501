    ...
    if isinstance(query, AggregationQuery):
        ...
        if aggregation_filter_end is not None:
            if aggregation_filter_end == "auto":
                aggregation_filter_end = get_mongo_pipeline_filter_end(
                    aggregation_query.aggregation_pipeline
                )
            filter_part = aggregation_query.aggregation_pipeline[
                :aggregation_filter_end
            ]
            transform_part = aggregation_query.aggregation_pipeline[
                aggregation_filter_end:
            ]
            aggregation_query.aggregation_pipeline = [
                *filter_part,
                ...
                [
                    {
                        "$facet": {
                            "metadata": [{"$count": "total"}],
                            "data": [*paginate_data, *projection_pipeline],
                        }
                    },
                ],
            )
        ...
        mongo_cursor = (
            aggregation_query.document_model.get_pymongo_collection().aggregate(
                pipeline,
        ...
        if is_limit_offset(raw_params):
            ...
            if cursor and cursor.startswith("prev_"):
                items = list(reversed(items))
            additional_data["next_"] = (
                str(items[-1].id) if next_link_available else None
            )
            additional_data["previous"] = f"prev_{items[0].id}" if items else None
