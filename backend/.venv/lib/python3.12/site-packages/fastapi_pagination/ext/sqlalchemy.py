...
Selectable: TypeAlias = (
    "Select[TupleAny] | TextClause | FromStatement[TupleAny] | CompoundSelect[TupleAny]"
)
...
def _paginate_from_statement(
    query: FromStatement[TupleAny], params: AnyParams
) -> FromStatement[TupleAny]:
...
def _limit_offset_flow(
    query: Selectable, conn: AnyConn, raw_params: RawParams
) -> LimitOffsetFlow:
...
@flow
def _cursor_flow(
    query: Selectable, conn: AnyConn, is_async: bool, raw_params: CursorRawParams
) -> CursorFlow:
    query = _prepare_query_for_cursor(query)

    if isinstance(query, TextClause):
        raise ValueError(
            "Cursor pagination cannot be used with raw SQL queries"
        )  # noqa: TRY004
    if isinstance(query, FromStatement):
        raise ValueError(
            "Cursor pagination cannot be used with FromStatement queries"
        )  # noqa: TRY004
...
def _sqlalchemy_flow(
    ...
    *,
    ...
    page = yield from generic_flow(
        ...
        params=params,
        inner_transformer=partial(
            _inner_transformer, query=query, unwrap_mode=unwrap_mode, unique=unique
        ),
