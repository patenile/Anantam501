...
def test_crud_raw_sql(setup_table):
    ...
    cur = conn.cursor()
    # Insert
    cur.execute(f"INSERT INTO {SCHEMA_NAME}.{TABLE_NAME} (name) VALUES ('Alice') RETURNING id;")
    row_id = cur.fetchone()[0]
    # Select
    cur.execute(f"SELECT name FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE id = %s;", (row_id,))
    assert cur.fetchone()[0] == 'Alice'
    # Update
    cur.execute(f"UPDATE {SCHEMA_NAME}.{TABLE_NAME} SET name = 'Bob' WHERE id = %s;", (row_id,))
    cur.execute(f"SELECT name FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE id = %s;", (row_id,))
    assert cur.fetchone()[0] == 'Bob'
    # Delete
    cur.execute(f"DELETE FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE id = %s;", (row_id,))
    cur.execute(f"SELECT COUNT(*) FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE id = %s;", (row_id,))
...
def test_unique_constraint(setup_table):
    ...
    with engine.connect() as conn:
        conn.execute(text(f"ALTER TABLE {SCHEMA_NAME}.{TABLE_NAME} ADD CONSTRAINT unique_name UNIQUE (name);") )
        # Insert a row
        conn.execute(text(f"INSERT INTO {SCHEMA_NAME}.{TABLE_NAME} (name) VALUES ('Frank');"))
        # Try to insert duplicate
        with pytest.raises(Exception):
            conn.execute(text(f"INSERT INTO {SCHEMA_NAME}.{TABLE_NAME} (name) VALUES ('Frank');"))
        # Cleanup constraint
        conn.execute(text(f"ALTER TABLE {SCHEMA_NAME}.{TABLE_NAME} DROP CONSTRAINT unique_name;"))

def test_data_types_and_casting(setup_table):
    engine = create_engine(DATABASE_URL)
    with engine.connect() as conn:
        # Insert integer as string, should cast
        conn.execute(text(f"INSERT INTO {SCHEMA_NAME}.{TABLE_NAME} (name) VALUES (:val);"), {"val": str(123)})
        result = conn.execute(text(f"SELECT name FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE name = '123';"))
        assert result.fetchone()[0] == '123'
        # Cleanup
        conn.execute(text(f"DELETE FROM {SCHEMA_NAME}.{TABLE_NAME} WHERE name = '123';"))
...
def test_error_handling(setup_table):
    ...
    with engine.connect() as conn:
        ...
        with pytest.raises(Exception):
            conn.execute(text(f"INSERT INTO {SCHEMA_NAME}.{TABLE_NAME} (name) VALUES (NULL);"))
