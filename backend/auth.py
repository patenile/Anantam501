...
pwd_context = CryptContext(schemes=["bcrypt_sha256"], deprecated="auto", bcrypt__variant="pybcrypt")
...
def get_password_hash(password):
    ...
    import sys
    debug_msg = f"[DEBUG] get_password_hash: type={type(password)}, value={repr(password)}\n"
    ...
    byte_len = len(safe_password.encode('utf-8'))
    # Ultra-detailed debug output
    ...
    debug_msg2 = (
        ...
        f"[DEBUG] Byte length: {len(safe_password_bytes)}\n"
    ...
    try:
        ...
        pass
    try:
        assert byte_len <= 72, f"Password passed to bcrypt is {byte_len} bytes, must be <= 72. Value: {repr(safe_password)}"
        return pwd_context.hash(safe_password)
    except ValueError as ve:
        import traceback
        tb = traceback.format_exc()
        error_msg = f"[ERROR] Exception in get_password_hash: {ve}\n{tb}\n[DEBUG] Password: {repr(safe_password)} (bytes: {byte_len})\n"
