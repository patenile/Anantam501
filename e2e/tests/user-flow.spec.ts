...
test.describe('User Registration and Login Flow', () => {
  test('should register a new user and login', async ({ page }) => {
    ...
    await expect(page.getByTestId('registration-message')).toBeVisible({ timeout: 10000 });
    await expect(page.getByTestId('registration-message')).toHaveText('Registration successful');
  ...
  test('should not register with invalid email', async ({ page }) => {
    ...
    await expect(page.getByTestId('registration-message')).toBeVisible({ timeout: 10000 });
    await expect(page.getByTestId('registration-message')).toHaveText('Invalid email');
  });
  ...
  test('should not login with wrong password', async ({ page }) => {
    ...
    // Wait for error message
    await expect(page.getByTestId('login-message')).toBeVisible({ timeout: 10000 });
